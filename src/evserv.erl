...
add_event(Name, Description, TimeOut) ->
  ...
  receive
    %% We can finish work here if receive error
...
loop(S = #state{}) ->
  receive
    ...
    {done, Name} ->
      case orddict:find(Name, S#state.events) of
        {ok, E} ->
          ...
          loop(S#state{events=NewEvents});
        error ->
          %% In case of simultaneously cancelation and it's time of event
          loop(S)
      end;
   ...
   shutdown ->
    ...
    code_change ->
      ?MODULE:loop(S); %% full definition to reload
...
valid_datetime({Date, Time}) ->
  try
    ...
    error:function_claus -> %% not like {{YYYY, MM, DD}, {HH, MM, SS}}
      false
  end;
